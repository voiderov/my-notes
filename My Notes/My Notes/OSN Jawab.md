...
1. **Station 5** (at 700 km)

1. **Station 8** (at 1150 km)

1. **Station 10** (at 1550 km)
...
1. We have rules:
    ...
    1. Rule b: 1-3 numbers, followed by 0-2 letters
    

1. Absolutely no duplicates
...
- **Jumlah Tak Hingga (S_inf):** `S_inf = a / (1 - r)`
...
1. It loops through the input vector **A** and populates **B** such that each element is the sum of the previous element in **B** and the current element in **A**. This creates a Prefix Sum Array. Specifically, `B[k]` stores the sum of the first `k` elements of **A**.

1. It initializes an output variable `out = 0`.

1. Using two nested loops, it evaluates all possible contiguous sub-segments of the array. For every starting index **i** and ending index **j**, it calls `GSI(B, i, j)`.

1. Because `GSI` returns `B[j] - B[i - 1]`, it is calculating the sum of the subarray from index **i-1** to **j-1** in the original array **A**. The function keeps track of the highest sum encountered using the `max()` function.
